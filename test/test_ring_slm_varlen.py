...
from ring_swa.streaming_llm.naive import naive_streaming_llm_attn_varlen
from ring_swa.streaming_llm.varlen import ring_streaming_llm_attn_varlen
...
def global_warmup(config: dict, warmup_times: int = 1):
    ...
    cu_seqlens = (
        torch.LongTensor([0] + list(seq_lens)).cuda().cumsum(-1).to(torch.int32)
    )
    total_seq_len = cu_seqlens[-1]
    local_seq_len = total_seq_len // world_size
    local_cu_seqlens = torch.LongTensor([0, local_seq_len]).cuda().to(torch.int32)
    ...
    for _ in range(warmup_times):
        ...
        do_local = torch.randn(
            local_seq_len, num_q_heads, head_dim, dtype=dtype, device="cuda"
        )

        o_warmup_flash = naive_streaming_llm_attn_varlen(
            q_local,
            k_local,
            v_local,
            window_size,
        ...
        o_warmup_ring = ring_streaming_llm_attn_varlen(
            q_local,
            k_local,
            v_local,
            window_size,
...
def generate_data(
    ...
    q[cu_seqlens[:-1], ...] = q[:1].clone()
    k[cu_seqlens[:-1], ...] = k[:1].clone()
    v[cu_seqlens[:-1], ...] = v[:1].clone()
    q.requires_grad = True
    k.requires_grad = True
    v.requires_grad = True
    do = torch.randn(total_seq_len, num_q_heads, head_dim, dtype=dtype, device="cuda")

    start_event.record()
    o = naive_streaming_llm_attn_varlen(
        q,
        k,
        v,
        window_size,
    ...
    q_chunks = [t.contiguous() for t in q.clone().detach().chunk(world_size, dim=0)]
    k_chunks = [t.contiguous() for t in k.clone().detach().chunk(world_size, dim=0)]
    v_chunks = [t.contiguous() for t in v.clone().detach().chunk(world_size, dim=0)]
    o_chunks = [t.contiguous() for t in o.clone().detach().chunk(world_size, dim=0)]
    do_chunks = [t.contiguous() for t in do.clone().detach().chunk(world_size, dim=0)]
    grad_q_chunks = [
        t.contiguous() for t in grad_q.clone().detach().chunk(world_size, dim=0)
    ]
    grad_k_chunks = [
        t.contiguous() for t in grad_k.clone().detach().chunk(world_size, dim=0)
    ]
    grad_v_chunks = [
        t.contiguous() for t in grad_v.clone().detach().chunk(world_size, dim=0)
    ]

    if dist.get_rank() == 0:
        print("\n" + "=" * 110)
        print(
            f"Test Config: dtype={str(dtype).split('.')[-1]}, cu_seq_lens={cu_seqlens.tolist()}, window_size={window_size}"
        )
        print(f"Q Heads: {num_q_heads}, KV Heads: {num_kv_heads}, Head Dim: {head_dim}")
        print(
            f"World Size: {world_size}, seq_len per rank: {total_seq_len // world_size}"
        )
...
def test_swa_varlen(config: dict):
    rank = dist.get_rank()
    world_size = dist.get_world_size()
    seq_lens, num_q_heads, num_kv_heads, head_dim = (
        config["seq_lens"],
    ...
    total_seq_len = cu_seqlens[-1]
    assert (
        total_seq_len % world_size == 0
    ), "total_seq_len must be divisible by world_size"
    ...
    local_seq_len = total_seq_len // world_size
    q_local = scatter_data(q_chunks, (local_seq_len, num_q_heads, head_dim), dtype)
    k_local = scatter_data(k_chunks, (local_seq_len, num_kv_heads, head_dim), dtype)
    v_local = scatter_data(v_chunks, (local_seq_len, num_kv_heads, head_dim), dtype)
    ref_o_local = scatter_data(
        ref_o_chunks, (local_seq_len, num_q_heads, head_dim), dtype
    )
    do_local = scatter_data(do_chunks, (local_seq_len, num_q_heads, head_dim), dtype)
    ref_grad_q = scatter_data(
        ref_grad_q_chunks, (local_seq_len, num_q_heads, head_dim), dtype
    )
    ref_grad_k = scatter_data(
        ref_grad_k_chunks, (local_seq_len, num_kv_heads, head_dim), dtype
    )
    ref_grad_v = scatter_data(
        ref_grad_v_chunks, (local_seq_len, num_kv_heads, head_dim), dtype
    )
    ...
    dist.barrier()
    start_event.record()
    o_local = ring_streaming_llm_attn_varlen(
        q_local,
        k_local,
        v_local,
        window_size,
    ...
    # UPDATED: Storing p99, p99.9, and max errors
...
if __name__ == "__main__":
    # torchrun --nproc_per_node=4 test/test_ring_slm_varlen.py
    ...
    try:
        ...
        test_configs = [
            {**base_config, "seq_lens": seq_lens, "window_size": window_size}
            for seq_lens in test_seq_lens
