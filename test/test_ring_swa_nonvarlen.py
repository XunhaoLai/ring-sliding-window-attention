...
from ring_swa.sliding_window.nonvarlen import ring_sliding_window_attn
...
def global_warmup(config: dict, warmup_times: int = 1):
    ...
    for _ in range(warmup_times):
        ...
        o_warmup_flash = flash_attn_func(
            q_local, k_local, v_local, window_size=(window_size, 0), causal=True
        )
        ...
        o_warmup_ring = ring_sliding_window_attn(
            q_local,
            k_local,
            v_local,
            window_size,
...
def generate_data(
    ...
    start_event.record()
    o = flash_attn_func(q, k, v, window_size=(window_size, 0), causal=True)
    end_event.record()
...
def test_swa_nonvarlen(config: dict):
    ...
    dist.barrier()
    start_event.record()
    o_local = ring_sliding_window_attn(
        q_local,
        k_local,
        v_local,
        window_size,
