...
from ring_swa.sliding_window.varlen import (
...
def global_warmup(config: dict, warmup_times: int = 1):
    ...
    local_max_seqlen = local_seq_len
    ...
    for _ in range(warmup_times):
        ...
        o_warmup_flash = flash_attn_varlen_func(
            q_local,
            k_local,
            v_local,
            local_cu_seqlens,
        ...
        o_warmup_ring = ring_sliding_window_attn_varlen(
            q_local,
            k_local,
            v_local,
            window_size,
...
def test_swa_varlen(config: dict):
    ...
    dist.barrier()
    start_event.record()
    o_local = ring_sliding_window_attn_varlen(
        q_local,
        k_local,
        v_local,
        window_size,
