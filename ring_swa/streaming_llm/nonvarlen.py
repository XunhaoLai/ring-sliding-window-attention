...
class RingStreamingLLMAttn(torch.autograd.Function):
    ...
    def forward(
        ...
        broad_sink_req = broadcast_tensor_to_group(sink_kv, rank_map[0], cp_group)
    ...
    def backward(ctx, do: torch.Tensor, *args):
        ...
        rank_map = get_group_local_to_global_rank_map(cp_group)
        sink_dkv = reduce_sum_tesnsor_to_rank(sink_dkv, rank_map[0], cp_group)
        if cp_local_rank == 0:
            dkv[:, :, :1].add_(sink_dkv)
...
def ring_streaming_llm_attn(
    ...
    Note:
        ...
        We assume that the i'th sequence chunk is placed in the global rank `rank_map[i % cp_size]`.
    """
